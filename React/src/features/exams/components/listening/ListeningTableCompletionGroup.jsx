...
const createPlaceholderHtml = (question) => `
  <span class="listening-test__table-placeholder">
    <span class="listening-test__table-placeholder-number">${question.number}</span>
    <span class="listening-test__table-placeholder-line">.......</span>
  </span>
`;

const createFallbackTableHtml = (questions = []) =>
  questions
...
const createTableHtml = (content, questions = []) => {
  if (!content) {
...
function ListeningTableCompletionGroup({
  group,
  currentPartTab,
  audioUrl,
  showPartTitle = true,
  ...
  const tableHtml = useMemo(
    () => createTableHtml(group.groupContent, group.questions),
  ...
  return (
    <section
      ...
      className="listening-test__table-completion"
    >
      {showPartTitle && <h2>Listening Part {currentPartTab}</h2>}
      {showAudio && <ListeningAudioPlayer audioUrl={audioUrl} />}

      <div
        className="listening-test__table-instructions"
        dangerouslySetInnerHTML={{ __html: instructionsHtml }}
      />

      <div className="listening-test__table-workspace">
        <div className="listening-test__table-card">
          <div
            className="listening-test__table-content"
...
export default memo(ListeningTableCompletionGroup);
