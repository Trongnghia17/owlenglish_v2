...
export default function TestLayout({
  ...
  // Get test ID and load/save notes to localStorage
  const testId = examData?.id || skillData?.id || sectionData?.id || 'default';
  ...
  useEffect(() => {
    if (testId) {
      localStorage.setItem(`test_notes_${testId}`, JSON.stringify(notes));
    }
  }, [notes, testId]);
      ...
      if (editingNoteId) {
        ...
        setNotes(notes.map(note => 
          note.id === editingNoteId 
            ? { ...note, content: currentNote.content, selectedText, updatedAt: new Date().toISOString() }
            : note
        ));
      } else {
        // Thêm ghi chú mới
                ...
                {notes.map((note, index) => (
                  ...
                  >
                      ...
                      <div className="test-layout__notes-item-info" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                        ...
                        >
                          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                            <path d="M2 4H14M6 7V11M10 7V11M3 4H13V13C13 13.2652 12.8946 13.5196 12.7071 13.7071C12.5196 13.8946 12.2652 14 12 14H4C3.73478 14 3.48043 13.8946 3.29289 13.7071C3.10536 13.5196 3 13.2652 3 13V4Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                          </svg>
                        </button>
                      </div>
                    </div>
                    <div className="test-layout__notes-item-body">
                      {note.selectedText && (
                        <h5 className="test-layout__notes-item-title">{note.selectedText}</h5>
                      )}
