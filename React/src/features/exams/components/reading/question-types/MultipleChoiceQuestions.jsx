    ...
    <div className={`reading-test__mc-layout ${showTips ? '' : 'reading-test__mc-layout--single'}`}>
            ...
            <div key={question.id} className="reading-test__mc-card">
              ...
              <div className="reading-test__mc-options">
                {questionOptions.map((option) => (
                  <label key={option.letter} className={`reading-test__mc-option ${selectedAnswers.includes(option.letter) ? 'selected' : ''}`}>
                    <input
      ...
      {showTips && (
        <aside className="reading-test__mc-tips">
          ...
          <p>+ Câu hỏi trắc nghiệm: chọn đáp án đúng (A/B/C/D) phù hợp với thông tin trong bài đọc.</p>
          <p>+ Đọc kỹ câu hỏi trước, sau đó quét bài đọc tìm thông tin tương ứng (scanning).</p>
          <p>+ Các lựa chọn có thể chứa thông tin đúng nhưng không trả lời được câu hỏi — chọn đáp án phù hợp nhất.</p>
          <p>+ Lưu ý số lượng đáp án: Choose ONE answer, Choose TWO letters. Đọc kỹ hướng dẫn.</p>
        </aside>
      )}
    </div>
  );
