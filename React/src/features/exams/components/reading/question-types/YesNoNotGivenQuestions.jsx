    ...
    <div className={`reading-test__mc-layout ${showTips ? '' : 'reading-test__mc-layout--single'}`}>
      ...
      {showTips && (
        <aside className="reading-test__mc-tips">
          ...
          <p>+ <strong>YES</strong>: Tác giả đồng ý/khẳng định với quan điểm trong câu hỏi.</p>
          <p>+ <strong>NO</strong>: Tác giả PHẢN ĐỐI/đối lập với quan điểm trong câu hỏi.</p>
          <p>+ <strong>NOT GIVEN</strong>: Bài đọc không đề cập đến quan điểm đó — không thể biết tác giả nghĩ gì.</p>
          <p>+ Lưu ý: NO ≠ NOT GIVEN. Chỉ chọn NO khi bài viết NÓI NGƯỢC QUAN ĐIỂM.</p>
        </aside>
      )}
    </div>
  );
...
export default memo(YesNoNotGivenQuestions);
