...
// Memoized instruction content to prevent re-render on timer ticks
const InstructionContent = memo(function InstructionContent({ content, fontSize }) {
  return (
    <div
...
// Memoized question content to prevent re-render on timer ticks
const QuestionContent = memo(function QuestionContent({ 
  ...
  const questionIsRecording = isRecording[question.id] || false;
  const questionRecording = recordings[question.id];

  return (
    <div className="speaking-test__question-item">
      ...
      <div className="speaking-test__recording-controls">
        ...
        <div className="speaking-test__action-buttons">
          <button
            className={`speaking-test__action-btn speaking-test__record-btn ${questionIsRecording ? 'recording' : ''}`}
            onClick={() => questionIsRecording ? onStopRecording(question.id) : onStartRecording(question.id)}
          >
        ...
        {questionRecording && (
          <div className="speaking-test__audio-wrapper">
            <audio controls src={questionRecording} />
          </div>
        )}
      </div>
    </div>
  ...
  // Only re-render if question content, recording state, or recordings change
  if (prevProps.question.id !== nextProps.question.id) return false;
  if (prevProps.question.content !== nextProps.question.content) return false;
  if (prevProps.qIndex !== nextProps.qIndex) return false;
  if (prevProps.isRecording[nextProps.question.id] !== nextProps.isRecording[nextProps.question.id]) return false;
  if (prevProps.recordings[nextProps.question.id] !== nextProps.recordings[nextProps.question.id]) return false;
...
const SpeakingTest = () => {
  ...
  const handleUpload = (questionId) => {
    console.log('Upload file for question:', questionId);
  };
      ...
      <div className="main_v2">
        <div className={`speaking-test__content ${fontSize !== 'normal' ? `speaking-test__content--${fontSize}` : ''}`}>
          ...
          {currentPartGroups.length > 0 && currentPartGroups[0].content && (
            <InstructionContent 
              content={currentPartGroups[0].content}
              fontSize={fontSize}
            />
          )}
